...
class WorkspaceView:
    ...
    def child_indexes(self) -> list[dict[str, Any]]:
        return self.store.child_indexes()
    ...
    def query(self, text: str, languages: list[str], parent: str, limit: int, offset: int) -> list[dict[str, Any]]:
        ...
        for child in self.store.child_indexes():
            ...
            if child_parent is None:
                continue
            child_rows = self._child_store(child).query(text, languages, child_parent, limit + offset, 0)
    ...
    def query_symbols(self, text: str, kind: str, limit: int, offset: int) -> list[dict[str, Any]]:
        ...
        for child in self.store.child_indexes():
            child_rows = self._child_store(child).query_symbols(text, kind, limit + offset, 0)
    ...
    def get_file(self, path: str) -> FileLookup:
        ...
        if item:
            return FileLookup(item, None, path)
        child, child_path = self.split_child_path(path)
        if not child:
            return FileLookup(None, None, path)
        item = self._child_store(child).get_file(child_path)
        if not item:
            return FileLookup(None, child, child_path)
        return FileLookup(self._prefix_file(child, item), child, child_path)

    def read_text(self, root: Path, path: str) -> str:
        child, child_path = self.split_child_path(path)
        if child:
            return self._read_child_text(child, child_path)
    ...
    def diff_filesystem(self, root: Path) -> dict[str, list[str]]:
        children = self.store.child_indexes()
        ...
        for child in children:
            child_diff = self._child_diff(child)
    ...
    def prefixed_files(self, child: dict[str, Any], files: list[dict[str, Any]] | None = None) -> list[dict[str, Any]]:
        rows = files if files is not None else self._child_store(child).all_files()
    ...
    def _child_diff(self, child: dict[str, Any]) -> dict[str, list[str]]:
        child_store = self._child_store(child)
        scan = SourceScanner(child["root"]).scan()
        indexed = {item["path"]: item for item in child_store.all_files()}
        current = {item.path: item for item in scan.records}
        return {
    ...
    def _read_child_text(self, child: dict[str, Any], path: str) -> str:
        root = Path(child["root"]).resolve()
        target = (root / path).resolve()
        if not str(target).startswith(str(root)):
            raise ValueError(f"path escapes project root: {child['path']}/{path}")
        return target.read_text(encoding="utf-8")
