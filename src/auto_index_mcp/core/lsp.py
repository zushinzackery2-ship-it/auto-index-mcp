...
class LspManager:
    ...
    def check(
        ...
        return format_check_result(diagnostics, len(opened) - len(missing_diagnostics), unchecked + len(missing_diagnostics), limit)
    ...
    def _open_targets(
        ...
        for item in files:
            ...
            try:
                ...
                session.open_document(uri, language_id(item), int(item.get("mtime_ns", 1)), text)
