...
class ServiceSemanticMixin:
    def semantic_search(
        ...
        similar symbols by cosine similarity. Requires an embedding backend
        (``AUTO_INDEX_EMBEDDING_MODEL``); without one it reports unavailable
        ...
        if indexer is None or service.store is None:
            return {
                ...
                "error": "embedding model not configured; set AUTO_INDEX_EMBEDDING_MODEL to an ONNX model directory",
                "items": [],
            }
    ...
    def embedding_status(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {"enabled": True, "model": indexer.backend.name, "vector_count": 0, "error": str(exc)}
