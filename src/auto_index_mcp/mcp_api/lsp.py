...
def register_lsp_tools(mcp: FastMCP, service: AutoIndexService) -> None:
    @mcp.tool()
    def auto_index_lsp_start(timeout_seconds: float = 10.0) -> str:
        """Start available LSP servers for the active indexed project (non-blocking)."""
    ...
    def auto_index_lsp_shutdown(timeout_seconds: float = 5.0) -> str:
        """Shut down all LSP servers for the active indexed project."""
